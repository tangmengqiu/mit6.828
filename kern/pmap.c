...
mem_init(void)
{
	...
	// Your code goes here:
	// 我日你妈 这个PTSIZE 写成了PGSIZE debug花了整整一天时间，艹！！！1
...
page_init(void)
{
	...
	// int num_iohole_alloc = (EXTPHYSMEM-IOPHYSMEM) / PGSIZE;
	// pages[0].pp_ref = 1;
	// pages[0].pp_link= NULL;
	// for (i = 1; i < npages; i++) {
	// 	if(i>=npages_basemem&& i < npages_basemem+num_iohole_alloc+num_extmem_alloc){
	// 		pages[i].pp_ref = 1;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	struct PageInfo * new_page_table = NULL;

	uint32_t page_dir_off = PDX(va);
	pde_t * dir_entry_ptr = pgdir + page_dir_off;
	if(!(*dir_entry_ptr & PTE_P)){
		if(create){
			new_page_table = page_alloc(1);
			if(new_page_table==NULL) return NULL;
			new_page_table->pp_ref++;
			*dir_entry_ptr =(page2pa(new_page_table)|PTE_P|PTE_W|PTE_U);
		}else{
			return NULL;
		}
	}
	page_off = PTX(va);
	page_base = KADDR(PTE_ADDR(*dir_entry_ptr));
